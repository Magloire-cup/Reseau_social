...
create policy "members can receive AI messages" on public.messages for insert to authenticated with check (sender_id is null and type = 'ai' and exists (select 1 from public.conversation_members where conversation_id = messages.conversation_id and user_id = auth.uid()));
